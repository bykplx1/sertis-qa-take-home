...
Tests assert intended behaviour rather than observed behaviour. Where a system violates its intent, the test fails and the failure is traceable to a numbered entry in a defect register. The pipeline goes red on those failures and blocks nothing: red and blocking are separate, and only the second would be wrong here. No branch protection references these jobs, so nothing is frozen over a defect in someone else's product — but a green check over a run that just reproduced nineteen of them would assert something untrue about the systems under test, and hide the defect summary behind a tick nobody has reason to click.
...
**Pipeline.** GitHub Actions. Two jobs, `e2e` and `api`, triggered on pull request and on push to `main`. The api job starts the local server before running. Both jobs upload their report and traces, and write a failure summary listing defect ids. Any non-passing test fails its job — failures, flakes, unexpected passes, and by-design defect reproductions alike (`.github/scripts/summarize-failures.js` exits 1; a broken run exits 2 and reads differently). Test failures remain non-blocking in the sense that matters: no branch protection or required check references these jobs, so a red run freezes no merge. What it does is refuse to certify as green a run whose content is a defect register. Two further things fail the job for reasons of their own: a broken run — the results file missing, unparseable, or describing zero executed tests, so a run that never actually executed cannot render as a green check — and a `typecheck` (`tsc --noEmit`) failure, since that is this repo's own code rather than a system under test. Expect `main` to sit red for as long as the systems under test stay defective; that is the intended reading, not a regression.
