...
**Pipeline.** GitHub Actions. Two jobs, `e2e` and `api`, triggered on pull request and on push to `main`. The api job starts the local server before running. Both jobs upload their report and traces, and write a failure summary listing defect ids. Test failures are non-blocking: this is a QA repository, the defects live in products it does not own, and evidence of a passing run is attached to the pull request rather than enforced by a gate. Two things do fail the job on purpose, in both jobs: a broken run — the results file missing, unparseable, or describing zero executed tests, so a run that never actually executed cannot render as a green check (`.github/scripts/summarize-failures.js`) — and a `typecheck` (`tsc --noEmit`) failure, since that is this repo's own code rather than a system under test.
